...
echo "🔍 Начинаю сбор кода проекта Stepper..."

# 1. Важные файлы конфигурации (если они существуют)
CONFIG_FILES=(
    "README.md"
    "Stepper/Info.plist"
    "Stepper/Stepper.entitlements"
    "Package.swift"
    "Podfile"
)
...
# 2. Ищем исходный код (.swift) и файлы локализации (.xcstrings) в папке Stepper
src_code=$(find Stepper -type f \( -name '*.swift' -o -name '*.xcstrings' \) \
    -not -path "*/Pods/*" \
    -not -path "*/.build/*" \
...
if [ -z "$files_to_read" ]; then
    echo "❌ Файлы не найдены. Убедись, что запускаешь скрипт из корня проекта (там, где лежит Stepper.xcodeproj)."
    exit 1
fi
...
if command -v pbcopy > /dev/null; then
    ...
    echo "✅ Готово! Весь код проекта Stepper скопирован в буфер обмена (macOS)."
